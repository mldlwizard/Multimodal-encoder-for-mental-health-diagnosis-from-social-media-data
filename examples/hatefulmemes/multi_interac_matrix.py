...
class CombinedModel(nn.Module):
    ...
    def forward(self, input1, input2):
        ...
        fusion_output = MultiplicativeInteractions2Modal(input_dims=[image_output.shape[1],text_output.shape[1]],output_dim=512,output='matrix').to(self.device)([image_output,text_output]).to(self.device)
...
os.chdir(home_path)
train_metrics.to_csv(home_path + "/results/metrics/" + str(idx) + "/hatefulmemes_multi_interac_matrix_config_" + str(idx) + ".csv",index=False)
